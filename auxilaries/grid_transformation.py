...
def modify_grid(grid, trans_dict):
    ...
    if transformation_op in grid_based_transformations:
        ...
        try:
            modified_grid = grid_based_transformations[transformation_op](
                grid, **transformation_params
            )
        ...
        filtered_nodes = [
            node
            for node in abstracted_graph.graph.nodes()
        ...
        if transformation_op == "extract":
            ...
            try:
                ...
                print(
                    f"Error applying transformation '{transformation_op}' with nodes '{nodes_to_keep}': {e}"
                )
            ...
            for node in filtered_nodes:
                ...
                if parameter_binding:
                    param_bound_node = getattr(abstracted_graph, parameter_binding)(
                        node, **filter_params
                    )
                    if param_bound_node:
                ...
                try:
                    ...
                    print(
                        f"Error applying transformation '{transformation_op}' on node '{node}': {e}"
                    )
...
def save_grids_and_comparison(
    original_grid, transformed_grid, transformation_details, path2save, idx
):
    ...
    space_between = 5
    comparison_image = PILImage.new(
        "RGB", (original_image.width * 2 + space_between, original_image.height)
    )
...
def prepare_and_save_transformed_data(
    grids, transformed_grids, transformation_details, output_folder="generated_llm_data"
):
    ...
    for index, (original_grid, transformed_grid) in enumerate(zip(grids, transformed_grids)):
        save_grids_and_comparison(
            original_grid, transformed_grid, transformation_details, path2save, index
        )
        ...
        with PILImage.open(image_path) as img:
            ...
            for i in range(img_array.shape[0]):
                for j in range(img_array.shape[1]):
                    ...
                    distances = [
                        np.linalg.norm(np.array(pixel) - np.array(color)) for color in color_map_rgb
                    ]
...
def _sample_and_apply_worker(
    q, no_of_trans, sample_folder, samples, transformation_ops, chosen_task
):
...
def sample_and_apply(
    ...
    def grid_equal(grid1, grid2):
        if np.array(grid1).shape != np.array(grid2).shape:
            return False
        return all(
            grid1[i][j] == grid2[i][j] for i in range(len(grid1)) for j in range(len(grid1[0]))
        )
    ...
    for idx in range(no_of_trans):
        ...
        while not changed:
            ...
            if filter_op == "filter_by_color":
                ...
                filter_params = {
                    "size": random.choice(["min", "max", "odd"]),
                    "exclude": random.choice([True, False]),
                }
            ...
            if transformation_ops is None:
                ...
                if not possible_trans_ops:
                    print(
                        f"No transformation operations available for abstraction '{abstraction}'. Skipping."
                    )
                    no_of_skips += 1
                    continue
                ...
                if idx >= len(transformation_ops):
                    ...
                    possible_trans_ops += [
                        ...
                        "shift_grid",
                        "truncate_grid",
                        "arbitrary_duplicate_grid",
                        "rotate_duplicate_grid",
                    ]  # Include new transformations
                    if not possible_trans_ops:
                        print(
                            f"No transformation operations available for abstraction '{abstraction}'. Skipping."
                        )
            ...
            if transformation_op == "update_color":
                ...
                transformation_params = {
                    "mirror_axis": random.choice(["diagonal", "horizontal", "vertical"])
                }
                ...
                transformation_params = {
                    ...
                    "upscale_type": random.choice(
                        ["pixel_based", "unique_colors", "standard", "other"]
                    ),
                ...
                transformation_params = {"color1": random.randint(0, 9)}
            elif transformation_op == "truncate":  # Added handling for truncate_grid
                transformation_params = {
                    "color1": random.randint(0, 9),
                    "color2": random.randint(0, 9),
                    "grid_size": random.randint(1, 10),
                    "truncate_type": random.choice(["position_based", "inferior_based"]),
                    "mirror": random.choice([True, False]),
                }
            elif (
                transformation_op == "arbitrary_duplicate"
            ):  # Added handling for arbitrary_duplicate_grid
                transformation_params = {
                    "mirror": random.choice([True, False]),
                ...
                transformation_op == "rotate_duplicate"
            ):  # Added handling for rotate_duplicate_grid
                import itertools
