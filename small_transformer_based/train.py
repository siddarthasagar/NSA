...
class SinusoidalPositionalEncoding2D(nn.Module):
    """
    Minimal 2D positional encoding stub.
    Applies 1D sinusoidal encoding to the sequence dimension to satisfy linting
    and optional 2D-PE usage without changing current behavior.
    """

    def __init__(self, n_embd, max_len=6500):
        super().__init__()
        # Reuse 1D encoding for now
        self.pe1d = SinusoidalPositionalEncoding(n_embd, max_len=max_len)
        ...
        if tta:
            ...
            for task_key, task_id, data_path in tqdm(
                ...
                if not os.path.exists(checkpoint_path):
                    ...
                    if os.path.exists(output_folder) and len(os.listdir(output_folder)) != len(
                        tta_data
                    ):
                        if (
...
def train(
    ...
    for batch_idx, (input_ids, output_ids) in enumerate(progress_bar):
        ...
        num_cls_tokens = (
            getattr(model, "module", model).num_cls_tokens
            if hasattr(model, "module")
            else model.num_cls_tokens
        )
                ...
                take = torch.cat(
                    [
                        ...
                        torch.full(
                            (pad_needed,),
                            pad_id,
                            dtype=row_clean.dtype,
                            device=row_clean.device,
                        ),
                    ]
                )
        ...
        if (batch_idx + 1) % args.print_iterations == 0:
            ...
            top3_predictions = [
                tokenizer.decode([int(token_id)]) for token_id in top3_logits.indices.cpu().numpy()
            ]
...
def main(data_path, epochs=1, batch_size=32, save_iterations=100):
    ...
    max_len = getattr(args, "max_length", 25600)
    ...
    train_eval_dataset = CustomDataset(train_eval_data, tokenizer, max_length=max_len)

    # Determine optimal number of DataLoader workers
    # Use multiple workers to prevent GPU starvation
    num_workers = 0
    pin_memory = False
    persistent_workers = False

    if device.type == "mps":
        # For MPS (Apple Silicon), use 2 workers - more causes memory pressure on unified memory
        num_workers = 2 if os.cpu_count() > 2 else 0
        pin_memory = False  # pin_memory not needed/supported on MPS
        persistent_workers = num_workers > 0
        print(f"Using {num_workers} DataLoader workers for MPS acceleration")
    elif device.type == "cuda":
        # For CUDA, standard approach
        num_workers = min(4, os.cpu_count() // 2) if os.cpu_count() > 2 else 0
        pin_memory = True
        persistent_workers = num_workers > 0
        print(f"Using {num_workers} DataLoader workers with pinned memory for CUDA")
    else:
        print("Using single-threaded DataLoader (CPU mode)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Batch size for training (default: auto-detect based on device)",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=25600,
        help="Maximum tokenized sequence length (default: 25600)",
    )
    ...
    if args.batch_size is not None:
        ...
        batch_size = 16  # Smaller batch size for MPS unified memory
    ...
    epochs_to_use = args.epochs if args.epochs is not None else 100
    main(
        args.data_path,
